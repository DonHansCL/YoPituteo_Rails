class RequestsController < ApplicationController
  ...
  def new
    @request = current_user.requests.build  #un usuario puede tener muchas request por eso va en plural con S al final
  end

  def create
    @request = current_user.requests.build(request_params)
    if @request.save
      redirect_to requests_path, notice: "Guardado"
    else
  ...
  def destroy
    @request.destroy
    redirect_to requests_path, notice: "Eliminado"
  end
  ...
  def is_authorised
    redirect_to root_path, alert: "No tienes los permisos para acceder" unless current_user.id == @request.user_id
  end
